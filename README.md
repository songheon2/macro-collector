...
| 파일 | 역할 |
|------|------|
| `collect.py` | API 수집 전용 진입점. KOSPI OHLCV·수급, 채권 수익률을 수집해 `outputs/raw/`에 저장 |
...
| `config.py` | 전체 상수·하이퍼파라미터·API 키·경로 중앙 관리. 모든 모듈이 이 파일에서만 값을 가져온다 |
...
| `bond_collector.py` | 한국은행 ECOS API를 통해 국고채(1Y~30Y)·통안채·회사채(AA-/BBB-) 수익률 및 기준금리 수집 |
...
| 파일 | 역할 |
|------|------|
| `manual_loader.py` | `inputs/` 하위 CSV를 읽어 날짜 인덱스 정규화(DatetimeIndex 변환, 시각 성분 제거) 후 `outputs/raw/`에 Parquet 저장. sentiment / global_macro / crypto 카테고리별 처리 |
...
| `feature_engineer.py` | 모델 입력용 파생 변수 생성. 로그 수익률, 20일 rolling 변동성, 금리 스프레드(10Y-1Y, 3Y-1Y, 신용 스프레드) 계산 |
...
| 국고채 | `gov_1y, gov_3y, gov_5y, gov_10y, gov_20y, gov_30y` | 한국은행 ECOS API |
| 통안채 | `msc_91d, msc_1y, msc_2y` | 한국은행 ECOS API |
| 회사채 | `corp_aa, corp_bbb` | 한국은행 ECOS API |
| 기준금리 | `base_rate` | 한국은행 ECOS API |
...
| `volatility_20d` | 로그 수익률 20일 rolling 표준편차 |
...
| 변수 | 설명 |
|------|------|
| `ECOS_API_KEY` | 한국은행 ECOS Open API 키. 미설정 시 `config.py` 내 기본값 사용 |
